...
def load_trajectories_from_eval(directory, movement_threshold=0.01):
    ...
    for i in successful_indices:
        ...
        slider_traj = np.stack([slider_actual.x, slider_actual.y, slider_actual.theta], axis=1)
...
def subsample_trajectory(trajectory, M):
    ...
    if L < M:
        raise ValueError(f"Trajectory length ({L}) is shorter than subsample count ({M})")
...
def plot_average_absolute_error(avg_abs_error):
    ...
    units = ["m", "m", "rad"]
    for i in range(D):
        axs[i].plot(time_steps, avg_abs_error[:, i], label=f"Dimension {i + 1}", color=f"C{i}")
