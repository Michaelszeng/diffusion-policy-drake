...
def get_run_traj_path(traj_number, main_folder, run_folder):
    traj_path = main_folder + "/" + run_folder + f"/run_{traj_number}/trajectory/traj_rounded.pkl"
    return traj_path
...
def plot_traj(traj_path, output_dir, traj_name):
    ...
    if type(traj) is PlanarPushingTrajectory:
        make_traj_figure(
            traj,
    ...
    elif type(traj) is SimplePlanarPushingTrajectory:
        plot_simple_traj(
            traj,
            filename=output_name,
