...
def construct_2d_plane_from_points(p1: npt.NDArray[np.float64], p2: npt.NDArray[np.float64]) -> Hyperplane:
    """
...
def calculate_convex_hull_cut_for_so_2(plane_1: Hyperplane, plane_2: Hyperplane) -> Hyperplane:
    ...
    cut_points = sorted([cut_point_1, cut_point_2], key=lambda p: np.arctan2(p[0], p[1]))
