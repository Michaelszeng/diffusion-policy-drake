...
class NonCollisionMode(AbstractContactMode):
    ...
    def create_source_or_target_mode(
        ...
        mode = cls.create_from_plan_spec(loc, config, mode_name, one_knot_point=True, terminal_cost=terminal_cost)
        if set_slider_pose:
            ...
            if self.cost_config.distance_to_object_socp is not None:
                ...
                for plane in planes:
                    for p_BP in self.variables.p_BPs:
                        ...
                        assert self.config.non_collision_cost.distance_to_object_socp is not None

                        c_2 = 1 / (self.config.non_collision_cost.distance_to_object_socp * self.dt)

                        phi = plane.dist_to(p_BP) - self.config.dynamics_config.pusher_radius
