...
def add_continuity_constraints_btwn_modes(
    ...
    lhs = outgoing_mode.get_continuity_terms(edge, "last", only_continuity_on_slider, continuity_on_pusher_velocities)
    rhs = incoming_mode.get_continuity_terms(edge, "first", only_continuity_on_slider, continuity_on_pusher_velocities)
...
class ContinuityVariables:
    ...
    def vector(self, include_pusher_velocities: bool = False) -> NpVariableArray | NpExpressionArray:
    ...
    def get_pure_variables(self, include_pusher_velocities: bool = False) -> NpVariableArray:
        """
        Function that returns a vector with only the symbolic variables (as opposed to having some be symbolic
        Expressions).
        """
        vars = sym.Variables()
...
class AbstractContactMode(ABC):
    ...
    @abstractmethod
    def get_variable_solutions(self, result: MathematicalProgramResult) -> AbstractModeVariables:
        pass
    ...
    def _get_vars_solution_for_vertex_vars(
        ...
        if isinstance(vars[0], sym.Expression):
            ...
            relevant_vertex_vars = vertex_vars[self.get_variable_indices_in_gcs_vertex(pure_vars)]
            ...
            return result.GetSolution(vertex_vars[self.get_variable_indices_in_gcs_vertex(vars)])
    ...
    def _get_var_solution_for_vertex_vars(
        ...
        return result.GetSolution(vertex_vars[self.get_variable_indices_in_gcs_vertex(np.array([var]))]).item()
