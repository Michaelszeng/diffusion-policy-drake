...
class Box2d(CollisionGeometry):
    ...
    def get_neighbouring_vertices(
        ...
        if location.pos == ContactLocation.FACE:
            raise NotImplementedError("Can't get neighbouring vertices for face contact")
        elif location.pos == ContactLocation.VERTEX:
            if location.idx == 0:
    ...
    def get_hyperplane_from_location(self, location: PolytopeContactLocation) -> Hyperplane:
        if location.pos == ContactLocation.VERTEX:
            raise NotImplementedError("Can't get hyperplane for vertex contact")
    ...
    def get_signed_distance(self, pos: npt.NDArray) -> float:
        ...
        # Left
        if pos_x <= -self.width / 2 and pos_y >= -self.height / 2 and pos_y <= self.height / 2:
            return -pos_x - self.width / 2
        # Right
        elif pos_x >= self.width / 2 and pos_y >= -self.height / 2 and pos_y <= self.height / 2:
            return pos_x - self.width / 2
        # Top
        elif pos_y >= self.height / 2 and pos_x >= -self.width / 2 and pos_x <= self.width / 2:
