...
class TPusher2d(CollisionGeometry):
    ...
    def get_planes_for_collision_free_region(self, idx: int) -> List[Hyperplane]:
        ...
        planes = []
        if idx == 0:
            planes.append(construct_2d_plane_from_points(self.vertices[0] + UL, self.vertices[0]))
            planes.append(construct_2d_plane_from_points(self.vertices[1], self.vertices[1] + UR))
            return planes
        if idx == 1:
            planes.append(construct_2d_plane_from_points(self.vertices[1] + UR, self.vertices[1]))
            planes.append(construct_2d_plane_from_points(self.vertices[2], self.vertices[2] + DR))
            return planes
        if idx == 2:
            planes.append(construct_2d_plane_from_points(self.vertices[2] + DR, self.vertices[2]))
            planes.append(construct_2d_plane_from_points(self.vertices[4], self.vertices[4] + DR))
            return planes
        if idx == 3:
            planes.append(construct_2d_plane_from_points(self.vertices[4] + DR, self.vertices[4]))
            planes.append(construct_2d_plane_from_points(self.vertices[5], self.vertices[5] + DL))
            return planes
        if idx == 4:
            planes.append(construct_2d_plane_from_points(self.vertices[5] + DL, self.vertices[5]))
            planes.append(construct_2d_plane_from_points(self.vertices[7], self.vertices[7] + DL))
            return planes
        if idx == 5:
            planes.append(construct_2d_plane_from_points(self.vertices[7] + DL, self.vertices[7]))
            planes.append(construct_2d_plane_from_points(self.vertices[0], self.vertices[0] + UL))
            return planes
        else:
    ...
    def normal_vecs(self) -> List[npt.NDArray[np.float64]]:
        normals = [-face.a for face in self.faces]  # Normal vectors point into the object
        return normals
    ...
    def get_faces_for_collision_free_set(self, location: PolytopeContactLocation) -> List[Hyperplane]:
        """
        Gets the faces that defines the collision free sets outside of each face.
        This function is hand designed for the object geometry!
        """
        if not location.pos == ContactLocation.FACE:
            raise NotImplementedError("Can only find faces for collisionfree regions for faces.")
        else:
        ...
        for face_idx in range(len(self.faces)):
            ...
            if np.all([plane.dist_to(pos) >= 0 for plane in planes]):
                ...
                if region_idx in [2, 4]:
                    ...
                    closest_face_idx = face_idxs_for_region[np.argmin([f.dist_to(pos) for f in faces_for_region])]
                    ...
                    return _make_jacobian(self.normal_vecs[face_idx], self.tangent_vecs[face_idx], pos)
    ...
    def get_signed_distance(self, pos: npt.NDArray) -> float:
        ...
        if dists[2] >= 0:  # we are inside box_2
            ...
            if not np.all(box_1_dists):
                raise RuntimeError("Finger is inside of box 1, but not all sdfs are negative. This must be a bug!")
            ...
            if not np.all(box_2_dists):
                raise RuntimeError("Finger is inside of box 2, but not all sdfs are negative. This must be a bug!")
            # we return the least penetration
