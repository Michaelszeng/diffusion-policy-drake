...
class SimulatedRealTableEnvironment:
    def __init__(
        self,
        desired_position_source: Diagram,
        ...
        self._robot_model_instance = self._plant.GetModelInstanceByName(self._robot_system.robot_model_name)
        self._slider_model_instance = self._plant.GetModelInstanceByName(self._robot_system.slider_model_name)
        ...
        if isinstance(self._desired_position_source, GamepadControllerSource):
            ...
            for camera_config in sim_config.camera_configs:
                ...
                image_writers[-1].DeclareImageInputPort(
                    ...
                    file_name_format=f"{image_writer_dir}/{camera_config.name}" + "/{time_msec}.png",
                    publish_period=0.1,
                    start_time=0.0,
                )
    ...
    def visualize_desired_pusher_pose(self, time_in_recording: float = 0.0):
        ...
        if self._pusher_goal_geometry is None:
            ...
            shape_geometry_id = self._robot_system._scene_graph.RegisterAnchoredGeometry(
                source_id,
                geom_instance,
            )
            ...
            self._robot_system._meshcat.SetObject(self._pusher_goal_geometry, pusher_shape, rgba=Rgba(*color))

        self._robot_system._meshcat.SetTransform(
            self._pusher_goal_geometry,
            self._sim_config.pusher_start_pose.to_pose(height / 2, z_axis_is_positive=True),
            time_in_recording,
        )

    def print_distance_to_target_pose(self, target_slider_pose: PlanarPose = PlanarPose(0.5, 0.0, 0.0)):
