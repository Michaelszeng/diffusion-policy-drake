...
class OptitrackFrameSource(LeafSystem):
    def __init__(
        ...
        if len(optitrack_frames) != len(optitrack_frame_times):
            raise ValueError(
                "The number of optitrack frames must be equal to the number of " + "optitrack frame times."
            )
...
class PosesToOptitrackFrameConverter(LeafSystem):
    def __init__(self, optitrack_object_ids: List[int]):
        ...
        self._pose_input_port = self.DeclareAbstractInputPort("poses", AbstractValue.Make(List[RigidTransform]))
        self._optitrack_frame_publisher = self.DeclareAbstractOutputPort(
            "optitrack_frame",
            lambda: AbstractValue.Make(optitrack_frame_t),
    ...
    def _convert_poses_to_optitrack_frame(self, context: Context, output: AbstractValue) -> None:
        # Read poses
...
class OptitrackObjectTransformUpdater(LeafSystem):
    ...
    def __init__(
        ...
        self.DeclareVectorOutputPort("object_positions", 7, self._get_current_object_positions)
    ...
    def _update_object_pose(self, context: Context, state: State) -> None:
        ...
        assert self._plant_context is not None, "The plant context must be set before the simulation is started!"

        # Get optitrack measurement
        optitrack_frame: optitrack_frame_t = self.get_input_port(self._optitrack_frame_index).Eval(context)
        optitrack_rigid_bodies: List[optitrack_rigid_body_t] = optitrack_frame.rigid_bodies
        ...
        iiwa_base_body = optitrack_rigid_bodies[optitrack_body_ids.index(self._optitrack_iiwa_id)]
        object_body = optitrack_rigid_bodies[optitrack_body_ids.index(self._optitrack_body_id)]
        ...
        X_world_optitrackBody: RigidTransform = self._X_world_iiwa @ X_iiwa_origin @ X_origin_optitrackBody

        X_world_plantBody: RigidTransform = X_world_optitrackBody @ self._X_optitrackBody_plantBody
        ...
        current_object_positions = self._plant.GetPositions(self._plant_context, self._object_instance_idx)
        if self._retain_z:
            object_translation[2] = current_object_positions[-1]
        # Creating a Quaternion might throw an error if the input is not perfectly
        # normalized
        current_object_RPY = (
            RotationMatrix(Quaternion(current_object_positions[:4] / np.linalg.norm(current_object_positions[:4])))
            .ToRollPitchYaw()
            .vector()
        )
        ...
        new_object_positions = np.concatenate((object_quaternion.wxyz(), object_translation))
        self._plant.SetPositions(self._plant_context, self._object_instance_idx, new_object_positions)
        self._object_positions = new_object_positions

    def _get_current_object_positions(self, context: Context, output: BasicVector) -> None:
        output.set_value(self._object_positions)

    @staticmethod
    def get_quaternion_from_optitrack_rigid_body(body: optitrack_rigid_body_t):
        return Quaternion(x=body.quat[0], y=body.quat[1], z=body.quat[2], w=body.quat[3])
    ...
    def __init__(
        ...
        if simulate:
            if lcm_publish_period is None:
                raise ValueError("The lcm_publish_frequency must be specified if simulate is True.")
        ...
        self._optitrack_object_transform_updater: OptitrackObjectTransformUpdater = builder.AddNamedSystem(
