...
class DiffusionPolicyController(LeafSystem):
    def __init__(
        ...
        self._image_deque_dict = {name: deque([], maxlen=self._obs_horizon) for name in self.camera_port_dict.keys()}
        ...
        if self._save_logs:
            self._logs = {
                ...
                "images": {name: [] for name in self.camera_port_dict.keys()},  # N x observation horizon x H x W x C
