...
class GcsPlannerController(LeafSystem):
    ...
    def __init__(
        ...
        self._collision_checker = CollisionChecker(
            self._sim_config.arbitrary_shape_pickle_path,
            self._sim_config.pusher_radius,
        )

        # Initialize state tracking variables
        self._traj_start_time = None  # Time when the GCS trajectory execution actually starts (when run_flag goes high)
        self._current_action = np.array([sim_config.pusher_start_pose.x, sim_config.pusher_start_pose.y])
        # During contact, the pusher is often in penetration with slider, but the plans generated by GCS Planner assume
        # no penetration, we add an offset equal to the penetration depth to prevent the pusher from "jumping" back out
    ...
    def DoCalcOutput(self, context: Context, output):
        ...
        time_since_traj_start = max(_time - self._traj_start_time, 1e-2)  # +1e-2 to avoid numerical issues
        ...
        self._detected_contact = self._detected_contact or self._collision_checker.check_collision(
            current_slider_pose, current_pusher_pose
        )
        ...
        if trial_step > self._last_plan_trial_step:
            ...
            solver_failed = new_traj is None or (traj_cost is not None and traj_cost > 1e6)
            if solver_failed:
