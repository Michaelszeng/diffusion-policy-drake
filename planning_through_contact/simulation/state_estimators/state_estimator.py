...
class StateEstimator(Diagram):
    """
    The State Estimator is the "internal" model that represents our knowledge of the real world and is not simulated. It
    contains a plant which is updated using a plant updater system. The plant itself is not part of the diagram while
    the updater system is.

    Used for real hardware experiments.
    """
    ...
    def __init__(
        ...
        if sim_config.camera_configs is not None:
            ...
            for camera_config in sim_config.camera_configs:
                ...
                builder.ExportOutput(
                    builder.GetSubsystemByName(f"rgbd_sensor_{camera_config.name}").color_image_output_port(),
        ...
        mbp_position_to_geometry_pose: MultibodyPositionToGeometryPose = builder.AddNamedSystem(
        ...
        # Export input ports
        builder.ExportInput(self._plant_updater.GetInputPort("robot_state"), "robot_state")
        builder.ExportInput(self._plant_updater.GetInputPort("object_position"), "object_position")

        # Export "cheat" ports
        builder.ExportOutput(self._scene_graph.get_query_output_port(), "query_object")
        builder.ExportOutput(self._plant_updater.get_state_output_port(), "plant_continuous_state")
        builder.ExportOutput(self._plant_updater.get_body_poses_output_port(), "body_poses")
        ...
        visualizer = MeshcatVisualizer.AddToBuilder(builder, self._scene_graph.get_query_output_port(), self.meshcat)
        zoom = 1.8
        camera_in_world = [
            sim_config.slider_goal_pose.x,
            (sim_config.slider_goal_pose.y - 1) / zoom,
    ...
    def _visualize_desired_slider_pose(self, desired_planar_pose: PlanarPose, time_in_recording: float = 0.0) -> None:
        ...
        if len(self._goal_geometries) == 0:
            ...
            for pose, geom_name in zip(poses, self._goal_geometries):
                self.meshcat.SetTransform(geom_name, desired_pose.multiply(pose), time_in_recording)

    def _visualize_desired_pusher_pose(self, desired_planar_pose: PlanarPose, time_in_recording: float = 0.0) -> None:
    ...
    def get_pusher_shape(self) -> DrakeCylinder:
        ...
        collision_geometries_ids = self._plant.GetCollisionGeometriesForBody(pusher_body)
