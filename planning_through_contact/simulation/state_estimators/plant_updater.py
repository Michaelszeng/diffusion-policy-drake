...
class PlantUpdater(LeafSystem):
    ...
    def __init__(self, plant: MultibodyPlant, robot_model_name: str, object_model_name: str):
        ...
        for i in range(self._plant.num_model_instances()):
            ...
            self.DeclareVectorOutputPort(
                ...
                self._plant.num_positions(model_instance) + self._plant.num_velocities(model_instance),
    ...
    def _get_state(
        ...
        state = self._plant.GetPositionsAndVelocities(self._plant_context, model_instance)
        output.set_value(state)

    def get_state_output_port(self, model_instance: ModelInstanceIndex = None) -> OutputPort:
