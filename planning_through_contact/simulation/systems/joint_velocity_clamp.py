    ...
    def DoCalcOutput(self, context, output):
        ...
        if time_step == 0 or (last_command == [-999] * self._num_positions).all():
            ...
            for i in range(self._num_positions):
                ...
                #     )
                #     logger.warn(
                #         f"({context.get_time()}) clamped acceleration {acceleration_mag} to "
                #         f"{self._joint_acceleration_limits[i]}"
                #     )
                if speed > self._joint_velocity_limits[i]:
                    # Above speed limit
                    joint_positions_clamped[i] = last_command[i] + self._joint_velocity_limits[i] * time_step * vel_sign
            ...
            if (joint_positions_clamped != joint_positions_commanded).any():
                logger.warn(
                    f"({context.get_time()}) clamped joint_positions_commanded deltas "
                    f"{abs((joint_positions_commanded - last_command) / time_step)}"
                )
