...
def _gcs_find_path_to_target(edges: List[GcsEdge], target: GcsVertex, u: GcsVertex) -> List[GcsVertex]:
...
def get_gcs_solution_path_vertices(
    ...
    active_edges = [edge for edge, flow in zip(gcs.Edges(), flow_results) if flow >= flow_treshold]
...
def _gcs_find_edges_to_target(edges: List[GcsEdge], target: GcsVertex, u: GcsVertex) -> List[GcsVertex]:
    current_edge = next(e for e in edges if e.u() == u)
    v = current_edge.v()
    target_reached = v == target
    if target_reached:
