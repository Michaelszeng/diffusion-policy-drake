"""
NOTE: this file is a modified version of the original station.py file in the manipulation package.

The reason we need to do this is we need an old version of manipulation (before it was upgraded to numpy 2.0) but we
need to make this update to make it compatible with the latest version of drake.

This only difference is the way sim_iiwa_driver is added to the added to the builder in _ApplyDriverConfigSim.
"""
...
class Scenario:
    ...
    plant_config: MultibodyPlantConfig = MultibodyPlantConfig(discrete_contact_approximation="sap")
...
def load_scenario(
    *,
    ...
    return LoadScenario(filename=filename, data=data, scenario_name=scenario_name, defaults=defaults)
...
def add_scenario(
    *,
    ...
    return AppendDirectives(filename=filename, data=data, scenario_name=scenario_name, defaults=defaults)
...
def _PopulatePlantOrDiagram(
    ...
    parser: Parser,
    scenario: Scenario,
    model_instance_names: typing.Optional[typing.List[str]],
    add_frozen_child_instances: bool = True,
    package_xmls: typing.List[str] = [],
    parser_preload_callback: typing.Optional[typing.Callable[[Parser], None]] = None,
    parser_prefinalize_callback: typing.Optional[typing.Callable[[Parser], None]] = None,
) -> None:
    """See MakeMultibodyPlant and MakeRobotDiagram for details."""
    if model_instance_names is None:
        assert (
            not add_frozen_child_instances
        ), "add_frozen_child_instances cannot be used without specifying model_instance_names."
    ...
    if model_instance_names is not None:
...
def MakeMultibodyPlant(
    scenario: Scenario,
    *,
    model_instance_names: typing.Optional[typing.List[str]] = None,
...
class _MultiplexState(LeafSystem):
    def __init__(self, plant: MultibodyPlant, model_instance_names: typing.List[str]):
        ...
        for name in model_instance_names:
            ...
            assert plant.num_positions(model_instance) == plant.num_velocities(model_instance)
...
class _DemultiplexActuation(LeafSystem):
    ...
    def CalcOutput(self, context, output, model_instance_index):
        ...
        output.SetFromVector(self._plant.GetActuationFromArray(model_instance_index, input))
...
def _ApplyDriverConfigSim(
    ...
    if isinstance(driver_config, IiwaDriver):
        ...
        model_instances = [sim_plant.GetModelInstanceByName(n) for n in model_instance_names]
        ...
        if len(model_instances) == 1:
            ...
            combined_state = builder.AddSystem(_MultiplexState(sim_plant, model_instance_names))
            combined_state.set_name(model_instance_name + ".combined_state")
            combined_input = builder.AddSystem(_DemultiplexActuation(sim_plant, model_instance_names))
            ...
            builder.Connect(controller.get_output_port_control(), combined_input.get_input_port())
            builder.ExportOutput(
                combined_state.get_output_port(),
                model_instance_name + ".state_estimated",
            )
...
def _ApplyPrefinalizeDriverConfigsSim(
    *,
    ...
    models_from_directives_map = dict([(info.model_name, info) for info in models_from_directives])
    for model_instance_name, driver_config in driver_configs.items():
...
def _ApplyCameraConfigSim(*, config: CameraConfig, builder: DiagramBuilder, lcm_buses: LcmBuses) -> None:
    ...
    builder.ExportOutput(camera_sys.color_image_output_port(), f"{config.name}.rgb_image")
    builder.ExportOutput(camera_sys.depth_image_32F_output_port(), f"{config.name}.depth_image")
    builder.ExportOutput(camera_sys.label_image_output_port(), f"{config.name}.label_image")


def MakeHardwareStation(
    scenario: Scenario,
    meshcat: typing.Optional[Meshcat] = None,
    *,
    package_xmls: typing.List[str] = [],
    hardware: bool = False,
    parser_preload_callback: typing.Optional[typing.Callable[[Parser], None]] = None,
    parser_prefinalize_callback: typing.Optional[typing.Callable[[Parser], None]] = None,
    prebuild_callback: typing.Optional[typing.Callable[[DiagramBuilder], None]] = None,
) -> Diagram:
    ...
    """
    if hardware:
        return _MakeHardwareStationInterface(scenario=scenario, meshcat=meshcat, package_xmls=package_xmls)
    ...
    # Add visualization.
    if meshcat:
        ApplyVisualizationConfig(scenario.visualization, builder, meshcat=meshcat, lcm_buses=lcm_buses)
...
def NegatedPort(builder: DiagramBuilder, output_port: OutputPort, prefix: str = "") -> OutputPort:
    ...
    """
    negater = builder.AddNamedSystem(f"sign_flip_{prefix}{output_port.get_name()}", Gain(-1, size=output_port.size()))
...
def _ApplyDriverConfigInterface(
    ...
    if isinstance(driver_config, IiwaDriver):
        ...
        iiwa_command_sender = builder.AddSystem(IiwaCommandSender(control_mode=control_mode))
        ...
        builder.ExportOutput(iiwa_state_mux.get_output_port(), model_instance_name + ".state_estimated")
...
def _WireDriverStatusReceiversToToPose(
    ...
    for i, model_instance_name in enumerate(model_instance_names):
        status_receiver = builder.GetSubsystemByName(f"{model_instance_name}.status_receiver")
...
def AddPointClouds(
    *,
    ...
    station: Diagram,
    builder: DiagramBuilder,
    poses_output_port: typing.Optional[OutputPort] = None,
    meshcat: typing.Optional[Meshcat] = None,
) -> typing.Mapping[str, DepthImageToPointCloud]:
    """
    Adds one DepthImageToPointCloud system to the `builder` for each camera in `scenario`, and connects it to the
    respective camera station output ports.
    ...
    Args:
        ...
        poses_output_port: (optional) HardwareStation will have a body_poses output port iff it was created with
        `hardware=False`. Alternatively, one could create a MultibodyPositionsToGeometryPoses system to consume the
        position measurements; this optional input can be used to support that workflow.

        meshcat: If not None, then a MeshcatPointCloudVisualizer will be added to the builder using this meshcat
        instance.
    ...
    for _, config in scenario.cameras.items():
        ...
        P = GetScopedFrameByName(plant, config.X_PB.base_frame) if config.X_PB.base_frame else plant.world_frame()
        ...
        if meshcat:
            ...
            point_cloud_visualizer = builder.AddSystem(MeshcatPointCloudVisualizer(meshcat, f"{config.name}.cloud"))
            builder.Connect(
